...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1].endswith(".json"):
        ...
        try:
            ...
            if debias_args.train_data == Nones:
                debias_args.train_data = data_args.train_file
            if debias_args.test_data == None:
                debias_args.test_data = data_args.test_file
            if debias_args.train_data_identity == None:
    ...
    if do_custom_model:
        ...
        gamma = 0
        result_dataset = OPTIMIZATION.debias_optimize_identity_diff_gamma(tokenizer, padding, max_seq_length, debias_args.baseline_metric_ind,
                                                         debias_args.baseline_metric_group, debias_args.save_dir, gamma, 
                                                         debias_args.baseline_fpr_train, debias_args.baseline_fnr_train)
