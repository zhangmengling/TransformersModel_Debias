...
from public_operation.debias_editor import AddedLayers
...
def main():
    ...
    if len(sys.argv) == 3 and sys.argv[1].endswith(".json"):
        ...
        print("-->debias_args", type(debias_args))
    ...
    if 'Llama' in model_args.model_name_or_path or 'llama' in model_args.model_name_or_path:
        ...
        # save_path = "./output/hate_speech_offensive_llama/"
    ...
    if training_args.do_predict:
        ...
        for predict_dataset, task in zip(predict_datasets, tasks):
            ...
            accuracy = compute_accuracy_metric(predictions, true_labels)
    ...
    if debias_parameters:
        ...
        if debias_args.do_debias:
            ...
            for gamma in gamma_parameters:
                ...
                # for param in Added_Layers.parameters():
                #     param.data = param.data.to(dtype=torch.float16)
                # Added_Layers = Added_Layers.half()
                ...
                editor, classifier = OPTIMIZATION.optimize_acc(tokenizer, padding, max_seq_length)
                OPTIMIZATION.get_metrics_with_editor(editor, classifier, path_directory, tokenizer, padding, max_seq_length)
            ...
            # result_dataset.to_csv("results/result_" + name + ".csv")

        ########## get baseline metrics on given Bert model only ##########
        if debias_args.do_get_baseline:
