...
class Optimization:
    def __init__(self, CustomModel, num_epochs, batch_size, learning_rate, target, privileged_label, threshold, gamma, train_data_name, test_data_name,
                 train_data_identity, test_data_identity, torch_dtype=torch.float32, metric=None, optimizer=None):
        ...
        # print(self.CustomModel.base_model.device)
    ...
    def get_predictions(self, model, classifier, dataset, tokenizer, padding, max_seq_length, if_identity=False):
        """
        ...
        if 'text' in dataset.columns:
            texts = dataset['text'].values.tolist()
        elif 'orig_text' in dataset.columns:
            texts = dataset['orig_text'].values.tolist()
        else:
            raise Exception("No text or orig_text column")
        ...
        if classifier != None:
            ...
            logits = self.CustomModel.custom_model['classifier'](output)
        logits = self.CustomModel.pooled_logits(logits, inputs)
    ...
    def get_predictions_all_identity1(self, model, classifier, dataset_identity, tokenizer, padding, max_seq_length, ID):
        ...
        if_identity = True: further output identity information
        RETURN:
        every_logits: prediction logits for texts batch of each identity in dataset_identity['orig_text']
        every_labels: prediction labels for each subgroup, a list
        overall_logits: prediction logits for dataset_identity['orig_text'] as a whole
        overall_labels: prediction labels for all texts
        all_base_labels: ground-truth labels for texts batch of each identity in dataset_identity['orig_text']
        """
        # ----------group metric on orig_text----------
        # datas = {'text': dataset_identity['orig_text'], 'label': dataset_identity['label']}
        # dataset = DataFrame(datas)
        ...
        for i in range(0, len(all_texts)):
            ...
            if classifier != None:
                ...
                logits = self.CustomModel.custom_model['classifier'](output)
            logits = self.CustomModel.pooled_logits(logits, inputs)
            # logits = logits.to(dtype=self.torch_dtype)
            # logits = active(logits)
            labels = [pro.index(max(pro)) for pro in logits.tolist()]
            every_logits.append(logits)
            every_labels.append(labels)
    ...
    def get_predictions_all_idi(self, model, classifier, dataset_identity, tokenizer, padding, max_seq_length):
        ...
        for i in range(0, num_columns - 1):
            ...
            # process the logits from [x, 128, 4096] to [x, 4096]
            logits = self.CustomModel.pooled_logits(logits, inputs)
            ...
            prob_logits = active_sigmoid(logits)
    ...
    def loss_function_cross_entropy_weight(self, logits, truth_labels):
        labels = torch.tensor(truth_labels)
    ...
    def normalization(self, input, min_value, max_value):
        ...
        min_value = torch.tensor(min_value, device=input.device).float()
        max_value = torch.tensor(max_value, device=input.device).float()
    ...
    def loss_function_sprt_individual(self, truth_labels, all_logits, all_labels, MinValue, MaxValue, gamma):
        """
        true_labels: group_truth label of the original dataset
        all_logits: logits of each dataset with certain identity, e.g., [logits of orig sample, ligits of idi1, logits of idi2, ...]
        all_labels: labels of each dataset with certain identity
        """
        ...
        acc_loss = self.loss_function_cross_entropy_weight(logits_orig, truth_labels)

        # # acc_loss
        # labels = torch.tensor([[(l + 1) % 2, (l + 2) % 2] for l in truth_labels], device=logits_orig.device)
        # labels = labels.float()
        # # acc_loss = criterian(logits_orig, labels)  # acc_loss on predictions with editor and updated classifier
        # acc_loss = criterian(logits_orig.squeeze(), labels.squeeze())
        # # return acc_loss, acc_loss, acc_loss
        ...
        label_true = torch.tensor([[(l + 1) % 2, (l + 2) % 2] for l in pred_labels], device=logits_idi1.device).float()
        label_false = torch.tensor([[(l + 2) % 2, (l + 1) % 2] for l in pred_labels], device=logits_idi1.device).float()
        ...
        n = torch.tensor(logits_orig.size(0), device=pred_false.device).float()  # device = device
        p0 = torch.tensor(self.threshold, device=pred_false.device).float()  # device = device
        ...
        gamma = torch.tensor(gamma, device=pred_false.device).float()
    ...
    def loss_function_sprt_fur(self, orig_labels, orig_identity_labels, logits_overall, identity_num, show_identity_num,
        ...
        # acc_loss
        # labels = torch.tensor([[(l + 1) % 2, (l + 2) % 2] for l in orig_labels], device=default_device)
        # labels = labels.to(logits_overall.dtype)
        # acc_loss = criterian(logits_overall, labels)  # acc_loss on predictions with editor and updated classifier
        acc_loss = self.loss_function_cross_entropy_weight(logits_overall, orig_labels)
        # normalization
        # acc_loss = self.normalization(acc_loss, 0.3, 0.8)
        ...
        # bias_loss
        label_1 = torch.tensor([[0, 1] for i in range(0, identity_num[0])], device=default_device, dtype=self.torch_dtype)
        label_0 = torch.tensor([[1, 0] for i in range(0, identity_num[0])], device=default_device, dtype=self.torch_dtype)
        # label_1 = label_1.to(logits_identity[0].device)
        # label_0 = label_0.to(logits_identity[0].device)
        # male
        pred_0_loss = torch.mean(criterian_each(logits_identity[0], label_0), 1)
        pred_1_loss = torch.mean(criterian_each(logits_identity[0], label_1), 1)
        minus = torch.sub(pred_0_loss, pred_1_loss)
        ...
        show_identity_num = torch.tensor(show_identity_num, dtype=self.torch_dtype)
        show_identity_num.to(default_device)
        p_hat = torch.div(DIFF_overall, show_identity_num)  # float()
        # n = torch.tensor(logits_overall.size(0), device=device).float()
        n = torch.tensor(self.batch_size, device=default_device, dtype=self.torch_dtype)
        p0 = torch.tensor(self.threshold, device=default_device, dtype=self.torch_dtype)
    ...
    def optimize_acc(self, tokenizer, padding, max_seq_length, save_dir):
        print("-->optimize_acc")
        '''
        fine-tune the editor and the classifier only based on accuracy loss
        purpose: instead of randomly initialize the editor, initialize the editor and calssifier with idealy accuracy 
        '''
        ...
        classifier = self.CustomModel.custom_model['classifier']
        optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': self.learning_rate},
                                     {'params': classifier.parameters(), 'lr': self.learning_rate}])  # SGD, Adam
        ...
        for epoch in range(self.num_epochs):
            ...
            start_index = 0
            end_index = 0
            while end_index < len(dataset_train) - 1:
                print("--> self.batch_size",  self.batch_size)
                end_index = min(len(dataset_train) - 1, start_index + self.batch_size)
                dataset_one_batch = dataset_train.iloc[start_index: end_index]
                orig_labels = dataset_one_batch['label'].values.tolist()
                labels = torch.tensor([[(l + 1) % 2.0, (l + 2) % 2.0] for l in orig_labels], device=device)
                # dataset_one_batch = self.dataset_process(dataset_one_batch)

                # calculate loss funcion on accuracy only
                logits, predict_labels = self.get_predictions(model, classifier, dataset_one_batch, tokenizer, padding, max_seq_length, False)
                # print("-->logits:", type(logits), logits.dtype, logits.shape, logits.device)
                # print("-->labels:", type(labels), len(labels), labels.device)
        ...
        # 画loss在迭代过程中的变化情况
        path_directory = save_dir + "acc/"
        os.makedirs(path_directory, exist_ok=True)

        model_path = path_directory + 'editor.pth'
        torch.save(model.state_dict(), model_path)
        model_path = path_directory + 'classifier.pth'
        torch.save(classifier.state_dict(), model_path)

        return model, classifier

    
    # def tdebias_optimize_identity_diff_gamma(self, tokenizer, padding, max_seq_length, baseline_metric_ind, baseline_metric_group, save_dir, gamma,
    #                                         baseline_fpr_train, baseline_fnr_train, if_save=False):

    #     dataset_train = self.train_data.sample(frac=1, random_state=999)
    #     dataset_test = self.test_data.sample(frac=1, random_state=999)
    #     dataset_identity = self.train_data_identity.sample(frac=1, random_state=999)
    #     dataset_identity_test = self.test_daa_identity.sample(frac=1, random_state=999)
    ...
    #     orig_classifier = copy.copy(self.CustomModel.custom_model['classifier'])
    #     classifier = self.CustomModel.custom_model['classifier']
    #     print("-->classifier", next(classifier.parameters()).device)
    #     optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': self.learning_rate},
    #                                  {'params': classifier.parameters(), 'lr': self.learning_rate}])
    ...
    #                 identity_num = [len(labels) for labels in orig_all_labels]
    #                 identity_pri_num = [labels.count(self.privileged_label) for labels in orig_all_labels]
    #                 show_identity_num = sum(1 for P in identity_num if P > 0)
    #                 show_identity_pri_num = sum(1 for P in identity_pri_num if P > 0)
    ...
    #                                                                          predict_logits, identity_num, show_identity_pri_num,
    #                                                                          MinValue, MaxValue, all_logits,
    #                                                                          baseline_fur, all_labels, gamma)
    #                 # changed show_identity_num to show_identity_pri_num
    ...
    #     #     max_length = max(len(lst) for lst in datas.values())
    #     #     padded_datas = {k: lst + [None] * (max_length - len(lst)) for k, lst in datas.items()}
    #     #     dataset = pd.DataFrame(padded_datas)
    #     # else:
    #     #     datas = {"train_ind": train_ind, "train_acc": train_acc, "train_f1": train_f1, "train_fpr": train_fpr,
    ...
    #     #     dataset = pd.DataFrame(padded_datas)
    #     if self.target == "individual":
    #         datas = {"test_ind": test_ind, "test_gender_ind": test_gender_ind, "test_religion_ind": test_religion_ind, "test_race_ind": test_race_ind,
    #              "test_acc": test_acc, "test_f1": test_f1, "test_fpr": test_fpr, "test_fnr": test_fnr}
    #         max_length = max(len(lst) for lst in datas.values())
    #         padded_datas = {k: lst + [None] * (max_length - len(lst)) for k, lst in datas.items()}
    #         dataset = pd.DataFrame(padded_datas)
    #     else:
    #         datas = {"test_ind": test_ind, "test_acc": test_acc, "test_f1": test_f1,
    #                  "test_fpr": test_fpr, "test_fnr": test_fnr, "test_fpr_all": test_fpr_all, "test_fpr_mean": test_fpr_mean,
    #                  "test_fpr_diff": test_fpr_diff, "test_fnr_all": test_fnr_all, "test_fnr_mean": test_fnr_mean,
    #                  "test_fnr_diff": test_fnr_diff}
    #         max_length = max(len(lst) for lst in datas.values())
    ...
    def debias_optimize_identity_diff_gamma(self, tokenizer, padding, max_seq_length, baseline_metric_ind, baseline_metric_group, save_dir, gamma,
                                            baseline_fpr_train, baseline_fnr_train, model=None, classifier=None):
        print("-->gamma", gamma)
        ...
        print("-->classifier", classifier)
        optimizer = torch.optim.Adam([{'params': model.parameters(), 'lr': self.learning_rate},
                                     {'params': classifier.parameters(), 'lr': self.learning_rate}])  # SGD
        # default
        # exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)
        # exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.5)
        exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=2, gamma=0.8)
        ...
        if self.target == "idi":
            ...
            # train_fpr_metric.append(train_fpr_metric)
            # train_fnr_diff.append(train_fnr_metric)
            ...
            # test_fpr_metric.append(test_fpr_metric)
            ...
            #          'train_fpr_metric': train_fpr_metric,	'train_fnr_mean': train_fnr_mean, 'train_fnr_diff': train_fnr_diff,
            #          'test_ind': test_ind, 'test_gender': test_gender_ind,
            #          'test_religion': test_religion_ind,
            #          'test_race': test_race_ind, 'test_cc': test_acc, 'test_f1': test_f1,
            #          'test_fpr': test_fpr, 'test_fnr': test_fnr, 'test_fpr_mean': test_fpr_mean,
            #          'test_fpr_metric': test_fpr_metric, 'test_fnr_mean': test_fnr_mean,
            #          'test_fnr_diff': test_fnr_diff
            #          }
            datas = {'train_ind': train_ind, 'train_gender': train_gender_ind, 'train_religion': train_religion_ind,
                     'train_race': train_race_ind, 'train_acc': train_acc, 'train_f1': train_f1,
        ...
        for epoch in range(self.num_epochs):
            ...
            while end_index < len(dataset_identity) - 1:
                ...
                if self.target == "individual":
                    ...
                    # check
                    # end_index = min(len(dataset_train) - 1, start_index + batch_size)
                    # dataset_one_batch = dataset_train.iloc[start_index: end_index]
                    # orig_labels = dataset_one_batch['label'].values.tolist()
                    # labels = torch.tensor([[(l + 1) % 2.0, (l + 2) % 2.0] for l in orig_labels], device=device)
                    ...
                    # individual bias mitigation
                    all_logits, all_labels = self.get_predictions_all_idi(model, classifier, dataset_identity_one_batch,
                                                                          tokenizer, padding, max_seq_length)
                    # orig_logits = self.get_predictions_all_basic(orig_classifier, dataset_identity_one_batch, tokenizer, padding, max_seq_length)

                    loss, acc_loss, bias_loss = self.loss_function_sprt_individual(orig_labels, all_logits, all_labels, MinValue, MaxValue, gamma)

                    # labels = torch.tensor([[(l + 1) % 2.0, (l + 2) % 2.0] for l in orig_labels], device=all_logits[0].device)
                    # loss = self.loss_function(all_logits[0], labels)
                    # acc_loss = loss
                    # bias_loss = loss

                    # check why the loss not work
                    # dataset_one_batch = dataset_train.iloc[start_index: end_index]
                    # orig_labels = dataset_one_batch['label'].values.tolist()
                    # labels = torch.tensor([[(l + 1) % 2.0, (l + 2) % 2.0] for l in orig_labels], device=device)
                    # logits, predict_labels = self.get_predictions(model, classifier, dataset_one_batch, tokenizer,
                    #                                               padding, max_seq_length, False)
                    # # all_logits, all_labels = self.get_predictions_all_idi(model, classifier, dataset_identity_one_batch, tokenizer, padding, max_seq_length)
                    # # logits = all_logits[0]
                    # loss = self.loss_function(logits, labels)
                    # acc_loss = loss
                    # bias_loss = loss
            ...
            #         train_fpr_mean.append(mean_fpr)
            #         train_fnr_mean.append(mean_fnr)
            #         train_fpr_metric.append(training_group_metirc)
            #         train_fnr_metric.append(train_fnr_metric)
            ...
            if self.target == "group":
                fpr_metric, fnr_metric, mean_fpr, mean_fnr, fpr, fnr = \
                    self.get_metrics_single(model, classifier, dataset_identity_test, tokenizer, padding,
                                            max_seq_length, 'fnr',
                                            fpr_epoch_test, fnr_epoch_test)
                if self.privileged_label == 1:  # FUR = FNR
                    print("-->group metric:", fnr_metric)
                else:  # FUR = FPR (self.privileged_label = 0)
                    print("-->group metric:", fpr_metric)
                if self.target == "group":
                    test_fpr_all.append(fpr)
                    test_fnr_all.append(fnr)
                    test_fpr_mean.append(mean_fpr)
                    test_fnr_mean.append(mean_fnr)
                    test_fpr_metric.append(fpr_metric)
        ...
        if self.target == "individual":
            ...
            #          "test_ind": test_ind, "test_gender_ind": test_gender_ind, "test_religion_ind": test_religion_ind, "test_race_ind": test_race_ind,
            #          "test_acc": test_acc, "test_f1": test_f1, "test_fpr": test_fpr, "test_fnr": test_fnr}
            datas = {"test_ind": test_ind, "test_gender_ind": test_gender_ind, "test_religion_ind": test_religion_ind,
            ...
            datas = {"test_acc": test_acc, "test_f1": test_f1,
                     ...
                     "test_fpr_metric": test_fpr_metric, "test_fnr_all": test_fnr_all, "test_fnr_mean": test_fnr_mean,
                     "test_fnr_metric": test_fnr_metric}
            dataset = pd.DataFrame(datas)
        print(dataset)
        return dataset
        # dataset.to_csv(path_directory + "figure/result.csv")
    ...
    def get_baseline_metrics(self, tokenizer, padding, max_seq_length):
        ...
        ##### training dataset #####
        print("#####training dataset#####")
        training_indi_metric, gender_metric, religion_metric, race_metric = self.get_metrics_single(None, None, dataset_identity, tokenizer, padding, max_seq_length, 'individual') 
        print("-->individual metric", training_indi_metric)
        train_ind.append(training_indi_metric)
        train_gender_ind.append(gender_metric)
        train_religion_ind.append(religion_metric)
        ...
        train_fpr_metric, train_fnr_metric, mean_fpr, mean_fnr, fpr, fnr = self.get_metrics_single(None, None, dataset_identity, tokenizer, padding, max_seq_length, 'group', fpr_train, fnr_train)
        
        train_fpr_all.append(fpr)
        train_fnr_all.append(fnr)
        train_fpr_mean.append(mean_fpr)
        train_fnr_mean.append(mean_fnr)
        train_fpr_metrics.append(train_fpr_metric)
        train_fnr_metrics.append(train_fnr_metric)

        ##### testing dataset #####
        print("#####testing dataset#####")
        metric, gender_metric, religion_metric, race_metric = self.get_metrics_single(None, None, dataset_identity_test, tokenizer, padding, max_seq_length, 'individual') 
        print("-->individual metric", metric)
        test_ind.append(metric)
        test_gender_ind.append(gender_metric)
        test_religion_ind.append(religion_metric)
        ...
        fpr_metric, fnr_metric, mean_fpr, mean_fnr, fpr, fnr = self.get_metrics_single(None, None, dataset_identity_test, tokenizer, padding, max_seq_length, 'group', fpr_test, fnr_test)
        test_fpr_all.append(fpr)
        test_fnr_all.append(fnr)
        test_fpr_mean.append(mean_fpr)
        test_fnr_mean.append(mean_fnr)
        test_fpr_metrics.append(fpr_metric)
        test_fnr_metrics.append(fnr_metric)

        datas = {"train_ind": train_ind, "train_acc": train_acc, "train_f1": train_f1, "train_fpr": train_fpr,
                 "train_fnr": train_fnr, "train_fpr_all": train_fpr_all, "train_fpr_mean": train_fpr_mean,
                 "train_fpr_metric": train_fpr_metric, "train_fnr_all": train_fnr_all, "train_fnr_mean": train_fnr_mean,
                 "train_fnr_metric": train_fnr_metric, "test_ind": test_ind, "test_acc": test_acc, "test_f1": test_f1,
                 "test_fpr": test_fpr, "test_fnr": test_fnr, "test_fpr_all": test_fpr_all,
                 "test_fpr_mean": test_fpr_mean,
                 "test_fpr_metric": test_fpr_metric, "test_fnr_all": test_fnr_all, "test_fnr_mean": test_fnr_mean,
                 "test_fnr_metric": test_fnr_metric}
    ...
    def get_metrics_with_editor(self, model, classifier, tokenizer, padding, max_seq_length):
        dataset_train = self.train_data.sample(frac=1, random_state=999)
        dataset_test = self.test_data.sample(frac=1, random_state=999)
        dataset_identity = self.train_data_identity.sample(frac=1, random_state=999)
        dataset_identity_test = self.test_data_identity.sample(frac=1, random_state=999)
        ...
        gender_fnr, religion_fnr, race_fnr = self.get_metrics_single(model, classifier, dataset_identity_test,
                                                                         tokenizer, padding, max_seq_length, 'group',
                                                                         FPR, FNR, True)

        print("-->mean fpr diff metric", test_fpr_metric)
        print("-->mean fnr diff metric", test_fnr_metric)

        individual_metric, gender_metric, religion_metric, race_metric = \
            self.get_metrics_single(model, classifier, dataset_identity_test, tokenizer, padding, max_seq_length, 'individual', FPR, FNR, True)
        print("-->individual metirc", individual_metric)
        print("-->gender_metric", gender_metric)
        print("-->religion_metric", religion_metric)
        print("-->race_metric", race_metric)
    ...
    def get_all_metrics(self, model, classifier, path_directory, tokenizer, padding, max_seq_length):
        ...
        for epoch in range(0, epochs_num):
            ...
            individual_metric, gender_metric, religion_metric, race_metric = \
                self.get_metrics_single(model, classifier, dataset_identity_test, tokenizer, padding, max_seq_length,
                                        'individual', FPR, FNR, True)
            print("-->individual metirc", individual_metric)
            print("-->gender_metric", gender_metric)
            print("-->religion_metric", religion_metric)
            print("-->race_metric", race_metric)
            test_ind.append(individual_metric)
            test_gender_ind.append(gender_metric)
            test_religion_ind.append(religion_metric)
            test_race_ind.append(race_metric)
        ...
        datas = { "test_ind": test_ind, "test_gender_ind": test_gender_ind, "test_religion_ind": test_religion_ind,
                 "test_race_ind": test_race_ind, "test_acc": test_acc, "test_f1": test_f1, "test_FPR": test_FPR, "test_FNR": test_FNR}
        dataset = pd.DataFrame(datas)
        return dataset
    ...
    def get_metrics_single_category(self, model, classifier, dataset_identity, tokenizer, padding, max_seq_length, metric,
        ...
        if metric == 'individual':
            ...
            print("-->FPR:{}, mean FPR:{}, mean FPR metric:{}".format(all_metrics['fpr'], np.mean(np.array(all_metrics['fpr'])), np.mean(np.array(fpr_diff))))
            print("-->FNR:{}, mean FNR:{}, mean FNR metric:{}".format(all_metrics['fnr'], np.mean(np.array(all_metrics['fnr'])), np.mean(np.array(fnr_diff))))
    ...
    def get_metrics_single(self, model, classifier, dataset_identity, tokenizer, padding, max_seq_length, metric,
        ...
        elif metric == 'fnr' or metric == 'fpr' or metric == 'group':
            all_metrics = {'acc': [], 'fpr': [], 'fnr': []}
            # ----------group metric on orig_text----------
            datas = {'text': dataset_identity['orig_text'], 'label': dataset_identity['label']}
            dataset = DataFrame(datas)
    ...
    def get_metrics_single_with_pred(self, dataset_identity, predict_labels, metric, baseline_fpr_train=None, baseline_fnr_train=None):
        ...
        if metric == 'individual':
            ...
            print("-->FPR:{}, mean FPR:{}, mean FPR metric:{}".format(all_metrics['fpr'],
                                                                    np.mean(np.array(all_metrics['fpr'])),
                                                                    np.mean(np.array(fpr_diff))))
            print("-->FNR:{}, mean FNR:{}, mean FNR metric:{}".format(all_metrics['fnr'],
